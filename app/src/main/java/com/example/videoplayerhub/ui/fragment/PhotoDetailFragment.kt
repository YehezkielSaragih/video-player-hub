...
import com.example.videoplayerhub.viewmodel.PhotoDetailViewModel
...
class PhotoDetailFragment : Fragment() {
    ...
    ): View? = inflater.inflate(R.layout.fragment_photodetail, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            viewModel.isFavorite.collectLatest { fav ->
                btnAddFav.isEnabled = !fav
                btnAddFav.text = if (fav) getString(R.string.already_in_favorites)
                else getString(R.string.add_to_favorites)
            }
        }

        // Check if already favorite
        viewModel.checkFavorite(photoId)

        btnAddFav.setOnClickListener {
            val fav = FavoritePhoto(
                id = photoId,
                author = photoAuthor,
            ...
            viewModel.addToFavorites(fav) { e ->
                e.printStackTrace()
